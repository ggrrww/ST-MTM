...
from .graphwavenet import GraphWaveNet_smae
...
class STDMAE(nn.Module):
    ...
    def __init__(self, dataset_name, pre_trained_tmae_path, pre_trained_smae_path, mask_args, backend_args):
        ...
        self.smae = Mask_ori(**mask_args)

        self.backend = GraphWaveNet_smae(**backend_args)
    ...
    def forward(self, history_data: torch.Tensor, long_history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, **kwargs) -> torch.Tensor:
        ...
        hidden_states = hidden_states_s   #(B,N,P,D)
        
        # enhance
        out_len=1
        hidden_states = hidden_states[:, :, -out_len, :]  #(B,N,D)
        y_hat = self.backend(short_term_history, hidden_states).transpose(1, 2).unsqueeze(-1)
