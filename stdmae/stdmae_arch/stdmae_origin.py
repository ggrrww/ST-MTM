...
from .mask import Mask_ori
from .graphwavenet import GraphWaveNet_ori


class STDMAE_ori(nn.Module):
    """Spatio-Temporal-Decoupled Masked Pre-training for Traffic Forecasting"""

    def __init__(self, dataset_name, pre_trained_tmae_path,pre_trained_smae_path, mask_args, mask_spatial_args, backend_args):
        super().__init__()
        ...
        self.smae = Mask_ori(**mask_spatial_args)

        self.backend = GraphWaveNet_ori(**backend_args)
    ...
    def forward(self, history_data: torch.Tensor, long_history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, **kwargs) -> torch.Tensor:
        ...
        y_hat = self.backend(short_term_history, hidden_states_t=hidden_states_t, hidden_states_s=hidden_states_s).transpose(1, 2).unsqueeze(-1)
