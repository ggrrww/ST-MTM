...
class GraphWaveNet(nn.Module):
    ...
    def __init__(self, num_nodes, supports, dropout=0.3, gcn_bool=True, addaptadj=True, aptinit=None, in_dim=2,out_dim=12,residual_channels=32,dilation_channels=32,skip_channels=256,end_channels=512,kernel_size=2,blocks=4,layers=2, **kwargs):
        ...
        self.fc_his_f = nn.Sequential(nn.Linear(96, 512), nn.ReLU(), nn.Linear(512, 256), nn.ReLU())
    ...
    def forward(self, input, hidden_states):
        ...
        hidden_states_t = self.fc_his_t(hidden_states)  #(B,N,7,D)
        hidden_states_t = hidden_states_t.transpose(2,3)
        hidden_states_t = self.fc(hidden_states_t).transpose(1,2) 
        
        # hidden_states_t = self.fc(hidden_states_t)
        # hidden_states_t = hidden_states.unsqueeze(0).expand(x.size(0), -1, -1, -1)  #(B,N,P,D)
        # hidden_states_t = self.fc_his_t(hidden_states_t)         # B, N, P, D
        # hidden_states_t = hidden_states_t.transpose(2,3)
        # hidden_states_t = self.fc(hidden_states_t).transpose(1,2)  #(B,N,D,1)
        skip = skip + hidden_states_t 
        
        # hidden_states_f = self.fc_his_f(hidden_states[:, :, 96:])        # B, N, D
        # hidden_states_f = hidden_states_f.transpose(1, 2).unsqueeze(-1)
        # skip = skip - hidden_states_t + hidden_states_f
