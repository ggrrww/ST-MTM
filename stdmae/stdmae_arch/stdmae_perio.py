...
class STDMAE_perio(nn.Module):
    ...
    def forward(self, history_data: torch.Tensor, long_history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, **kwargs) -> torch.Tensor:
        ...
        # short_term_future = future_data     # [B, L, N, 1]
        batch_size, _, num_nodes, _ = history_data.shape

        hidden_states_t = self.tmae(long_history_data[..., [0]])  #(B,N,7,P,D)
        hidden_states = hidden_states_t
        # hidden_states_f = self.tmae(short_term_future)
        # hidden_states_s = self.smae(long_history_data[..., [0]])
        # hidden_states=torch.cat((hidden_states_t,hidden_states_s),-1)
        # hidden_states=torch.cat((hidden_states_t,hidden_states_f),-1)
        
        # enhance
        out_len=1
        hidden_states = hidden_states[:, :, :, -out_len, :]  #(B,N,7,D)
