...
class TQpretrain(nn.Module):
    def __init__(self, num_nodes, input_len, in_channel, embed_dim, hidden_dim, 
        ...
        self.encoder = nn.MultiheadAttention(embed_dim=self.embed_dim, num_heads=4, batch_first=True, dropout=0.5)
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor = None, batch_seen: int = None, epoch: int = None, **kwargs):
        ...
        source_emb = self.linear_TE(source.transpose(1,2))  #(B,N,D)
        
        if self.mode == "pre-train":
            hidden_state = self.encoder(query=pose_emb, key=source_emb, value=source_emb)[0]
            for layer in self.decoder:
                hidden_state = layer(hidden_state)  #(B,N,D)
            out = self.out_proj(hidden_state)  #(B,N,T)
            out = out.transpose(1, 2)  #(B,T,N)
            
            return out, future_data[..., 0] 
        else:
