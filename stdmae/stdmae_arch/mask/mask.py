...
class Mask(nn.Module):
    ...
    def __init__(self, patch_size, in_channel, embed_dim, num_heads, mlp_ratio, dropout, week, mask_ratio, encoder_depth, decoder_depth,spatial=False, mode="pre-train"):
        super().__init__()
    ...
    def encoding(self, long_term_history, mask=True):
        ...
        if mask:
            ...
            if not self.spatial:
                #(4,170,1,2016)
                B, N, _, _ = long_term_history.shape
                patches = long_term_history.reshape(B, N*self.week, 1, -1)  #(B,N,7,288)

                patches = self.patch_embedding(patches)  # (4, 7*N, 96, 24)
    
                patches = patches.transpose(-1, -2)  # B, N, P, d
                batch_size, num_nodes, num_time, num_dim  =  patches.shape  #(B,7N,P,D)
                ...
                hidden_states_unmasked = self.encoder(encoder_input)  #（B,7N,P,D)
                hidden_states_unmasked = self.encoder_norm(hidden_states_unmasked).view(batch_size, num_nodes,  -1, self.embed_dim)

        else:
            batch_size, num_nodes, _, _ = long_term_history.shape
            patches = long_term_history.reshape(batch_size, num_nodes*self.week, 1, -1)  #(B,N*7,1,288)
            ...
            hidden_states_unmasked = self.encoder_norm(hidden_states_unmasked).view(batch_size, num_nodes, self.week, -1, self.embed_dim)# B, N, P, d
    ...
    def decoding(self, hidden_states_unmasked, masked_token_index):
        ...
        if self.spatial:
            ...
            batch_size, num_nodes, num_time, _ = hidden_states_unmasked.shape  # [4, 1190, 18, 96]
            ...
            # prediction (reconstruction)
            reconstruction_full = self.output_layer(hidden_states_full.view(batch_size, num_nodes, -1, self.embed_dim))  #(B,7N,P,L)
    ...
    def get_reconstructed_masked_tokens(self, reconstruction_full, real_value_full, unmasked_token_index,
        ...
        if self.spatial:
            ...
            batch_size, num_nodes, num_time, _ = reconstruction_full.shape  #[4, 1190, 24, 12]

            reconstruction_masked_tokens = reconstruction_full[:, :, len(unmasked_token_index):, :]     # B, 7N, r*L, d
            reconstruction_masked_tokens = reconstruction_masked_tokens.view(batch_size, num_nodes, -1).transpose(1, 2)     # B, r*P*l, 7N
            #  print(reconstruction_masked_tokens.shape) [4, 72, 1190]

            # B, N, 1, L * P
            _, ori_N, _, _ = real_value_full.shape
            label_full = real_value_full.squeeze(-2)  #(B,N,L*P)
            label_full = label_full.reshape(batch_size, ori_N, self.week, -1)
            label_full = label_full.reshape(batch_size, ori_N*self.week, -1, self.patch_size)  #(B,7N, 24,12)

            label_masked_tokens = label_full[:, :, masked_token_index, :].contiguous() # B, 7N, r*P, d
