...
class Perio_Pretraining_Transformer(nn.Module):
    def __init__(self, num_nodes, input_len, in_channel, embed_dim, hidden_dim, 
        ...
        self.out_proj = nn.Linear(self.input_len*self.embed_dim, self.input_len)
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor = None, batch_seen: int = None, epoch: int = None, **kwargs):
        ...
        pose_emb = pose_emb.transpose(1, 2)  #(B,N,T,D)
        ...
        if self.mode == "pre-train":
            ...
            for layer in self.decoder:
                hidden_states = layer(hidden_states)  #(B,N,T,D)
            reconstruction = hidden_states
            out = reconstruction.view(B, N, -1)  #(B,N,T*D)
            ...
            for layer in self.encoder:
                pose_emb = layer(pose_emb)
            hidden_states = pose_emb #(B,N,T,D)
